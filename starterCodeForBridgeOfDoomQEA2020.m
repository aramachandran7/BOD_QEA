...
sub_imu = rossubscriber('/imu');
n = 1;
stopMsg.Data = [-.3, .3];
send(pub, stopMsg);

% sub_enc = rossubscriber('/encoder/')
% while 1
%     encoderM = recieve(sub_enc)
%     
%     disp(encoderM)
%     
%     
...
while 1
    ...
    imuMessage = receive(sub_imu);
    
    rs(n,:) = norm([imuMessage.AngularVelocity.X,imuMessage.AngularVelocity.Y]);
%     disp(rs)
    disp(cumtrapz(rs))
%     if cumtrapz(rs)>(pi/10)
%         disp('cumtrapz suprassed threshold')
%         break
%     end 
%     n= n+1;
%     if n==30
